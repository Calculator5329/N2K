...
- [ ] <!-- workspace:id=work:ce370265-f6d3-5f41-a562-47757e628705 --> **Deploy HEAD to n2k-almanac-v3.web.app** (owner). The 2026-09-13 site sweep tested a stale build: folio numerals still swapped live although fixed 2026-07-10, plus the four Lookup/Competition majors fixed 2026-09-13.
- [x] <!-- workspace:id=work:4a5b42d0-5df1-5eed-81ca-e823c067d312 --> **Epoch dates in Receipt, Spreadsheet, Polaroid and Manuscript mastheads** render "1970-01-01" on the live build; confirm against HEAD and fix the date source if it persists. (done 2026-09-25: reproduced on HEAD; `n2kLoader.loadIndex` hardcoded `new Date(0)`. Now the build injects the date `standard.n2k` was last committed to the repo (not its bake date), so all four show 2026-04-20. Not deployed.)
- [x] <!-- workspace:id=work:c1013aa5-5d7b-54cf-a9cb-c7d859faf9a0 --> **Receipt edition Lookup right column spills past the paper edge** at 1280 wide. (done 2026-09-25: the Lookup grid prints as one column on the receipt; nothing crosses the paper at 1280, 1440 or 390. Not deployed.)
- [x] <!-- workspace:id=work:24fbae85-9bf9-53b8-b97b-7bc628f8b80b --> **Only the Lookup view is in the URL hash**: Back leaves the site from Competition, Library and Play, reload always lands on Lookup, and non-Lookup views cannot be linked. Done 2026-09-25: non-Lookup views carry `view=competition|library|play` in the hash (`web/src/stores/viewHash.ts`), tab clicks push history, Lookup links keep their exact format. Guarded by `web/tests/viewHash.test.ts` and `web/e2e/view-hash.spec.ts`.
- [x] <!-- workspace:id=work:243876df-ef9f-59ec-a216-dde692ffe4b0 --> **Play race board has no move validation** and the clear bonus is out of scale (36 cells worth 5,328 scored 60,317); Easy and Standard bots knock zero cells in 60 seconds. *(done 2026-09-25)* Knocks are typed equations checked by the shared `claimRefusal`; a clear adds up to half the board (that example now 7,757); the bot walks easiest first, so Easy knocks 2 to 3 cells and Standard 4 to 6, and Master slows to speed 6 (4 to 15). See changelog.
- [ ] <!-- workspace:id=work:8ef5f8a8-27ee-5370-948a-5480a74dde6b --> **Exact rational check for typed knocks.** `claimRefusal` and the typed-equation parser compare the left-to-right value to the total with `FLOAT_EQ_EPSILON` (1e-9), so a long division chain could pass or fail on float error. Evaluate with integer numerator/denominator pairs instead and drop the epsilon on that path.
- [x] <!-- workspace:id=work:17af5ab9-196d-56f7-b57b-654167fc9501 --> **Competition small fixes**: Escape does not close Manage Phases; Generate is a silent no-op on a phase with no boards and drops boards with 0 bouts without warning; "Remove" on board headers clips to "REMOV" at 390px; "1 cells knocked" plural. (done 2026-09-25: all four fixed; Generate names what it skipped under the toolbar. Not deployed.)
