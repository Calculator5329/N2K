...
- **Why:** Compose already generates balanced multi-board competitions
  and exports PDF, and that IS a classroom product. Teachers are the most
...
- **Why:** Mental-math training is a phone habit. The app is already
  a static SPA with immutable-cached data blobs, so most of the work is
...
- **Impact/effort:** Medium-high / M
- **Why:** "Race the same board as me" via URL, giving multiplayer feel with
...
- **Why:** Natural extension of the Competition > Phase > Board > Bout
  model, since a bracket is a phase graph. Needs accounts (6) first to
  matter.
...
- **Why:** Fun demo of the themes-as-data architecture and an AI
  ...
  prune removed the JSON theme registry, so themes are TS code again
  (`web/src/core/themes.ts`). This now needs schema work plus an
