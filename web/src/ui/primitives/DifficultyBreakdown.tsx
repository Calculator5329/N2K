...
import { STANDARD_MODE, AETHER_MODE } from "@solver/core/constants.js";
...
> = {
  ...
  arityPenalty: {
    label: "Extra dice in play",
    description:
      "Building from four or five dice instead of three means more tracking.",
  },
  negativeBasePenalty: {
    label: "Negative dice",
    description:
      "Reasoning about negative bases (and the signs they introduce) adds load.",
  },
  hugeExponentPenalty: {
    label: "Sky-high exponents",
    description:
      "Pushing a die to a very large power demands rare mental arithmetic.",
  },
};
...
export function DifficultyBreakdown({
  ...
  const view = useMemo(() => {
    try {
      ...
      // Pick the mode whose dice range and arity actually accept the
      // parsed equation: Standard fits 3-arity tuples in 2..20; anything
      // wider (negatives, arity 4/5, dice > 20) is Æther by definition.
      const allStandard =
        parsed.dice.length === 3 &&
        parsed.dice.every((d) => d >= 2 && d <= 20);
      const mode = allStandard ? STANDARD_MODE : AETHER_MODE;
      const breakdown = difficultyBreakdown(parsed, mode);
