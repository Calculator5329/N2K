 ...
 * tweak parameters, set per-cell overrides on a 6×6 grid (which auto-previews
 * the generated cells), and remove the board.
 *
    ...
    <div className="border border-ink-100/20 bg-paper-50 px-6 py-5">
      ...
      <div className="grid grid-cols-12 gap-y-6 md:gap-6">
        ...
        <div className="col-span-12 md:col-span-5 space-y-4 min-w-0">
          ...
          {board.errorMessage && (
            <div className="pt-1 text-[11px] font-mono text-oxblood-500">
              {board.errorMessage}
            </div>
          )}
        </div>

        {/* RIGHT — 6×6 override grid (shows the live sample) */}
...
const RandomParams = observer(function RandomParams({
  ...
  const { min, max } = store.cellBounds;
  return (
...
const PatternParams = observer(function PatternParams({
  ...
  const { max: cellMax } = store.cellBounds;
...
const RoundsField = observer(function RoundsField({
  ...
  return (
    <NumberField
      ...
      min={1}
      max={20}
      onChange={(v) => store.updateBoard(board.id, { bouts: v })}
    />
  );
});
