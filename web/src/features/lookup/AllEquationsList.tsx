...
function ResultList({
  ...
  // Sum of multiplicities = the original raw equation count before
  // canonical-form collapse. Show it parenthetically so power users
  // can still see how many "ways" the cell has, while the headline
  // count reflects the meaningfully-distinct shapes.
  const rawCount = solutions.reduce((s, c) => s + c.multiplicity, 0);
  const collapsed = rawCount !== total;
    ...
    <div>
      <div className="label-caps text-ink-100 mb-2 flex items-baseline justify-between">
        <span>
          ...
          {collapsed && (
            <span className="text-ink-100/70 normal-case ml-2 font-mono text-[10px] tabular">
              ({rawCount} orderings)
            </span>
          )}
        </span>
        ...
        {solutions.slice(0, shown).map((s, i) => (
          ...
          >
            <div className="flex items-baseline gap-3 min-w-0">
              ...
              {s.multiplicity > 1 && (
                <span
                  className="font-mono text-[10px] text-ink-100/70 tabular shrink-0"
                  title={`${s.multiplicity} commutatively-equivalent orderings collapse to this canonical form`}
                >
                  ×{s.multiplicity}
                </span>
              )}
