 ...
 * of truth; the on-disk `standard.n2k` blob only covers tuples in this
 * range (no dice with value 1, since `isLegalDiceTriple` rejects them).
 * Locking the picker / hash schema bounds to the same values prevents
 * the user from typing a triple that would 404 against the blob.
 */
...
function clampDie(value: number): number | null {
  ...
  return Math.max(DICE_MIN, Math.min(DICE_MAX, Math.round(value)));
}
