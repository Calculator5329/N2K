 ...
 * dice rolls) into a binary deliverable — currently PDF only. Lives in
 * the service layer per the project's three-layer rule: no MobX, no
 * UI awareness, no dependency on the live `CompositionStore`
 * instance. Callers lower their store down to a plain
 * `CompositionExportData` envelope and pass that in.
 *
 ...
 *   page  N+1+  "Stats summary" — full per-bout difficulty + expected
 ...
 *
 * Bundle hygiene: `jspdf` (+ html2canvas, DOMPurify) adds ~220KB to
 * the bundle. It is loaded via dynamic `import()` from a sibling impl
 * file so it only ships to users who actually click the export
 * button. The public surface here stays lightweight: just types, the
 * `downloadBlob` helper, and one thin promise-returning entry point.
 */
...
export interface ExportBoard {
  ...
  /**
   * Optional phase signifier prefix rendered above the title in the
   * PDF eyebrow row. v3.2: callers populate this with the phase name
   * so a multi-phase deck reads "Phase 2 — Board 1" on paper without
   * losing the flat one-page-per-board layout.
   */
  readonly phaseLabel?: string;
  /** Number of bouts shown in the rolls table. */
...
export interface CompositionExportData {
  ...
  /** Optional human competition name (added v3.2). */
