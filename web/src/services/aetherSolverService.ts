 ...
 * (min 1). `sweepAdvanced` (full target sweep, fixed arity) is
 * dispatched across the pool with least-busy assignment so concurrent
 * UI flows actually use multiple cores.
 *
 * The API returns a Promise that resolves once the worker reply lands.
 ...
 * tuple in a single brute-force pass. Operator/exponent enumeration is
 * amortized across all targets, so this is dramatically cheaper than
 * solving each target independently.
 *
 * `dice` defines both the tuple identity and the arity. If the caller
 * wants auto-arity behaviour they should call this for every relevant
 * subset themselves and reduce.
 */
